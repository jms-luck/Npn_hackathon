...
import { api, apiBlob, apiResponse } from "./api/client";
...
function Shell({ children }) {
  ...
  return <div className="shell">
    <aside className={open ? "sidebar open" : "sidebar"}>
      <div className="brand"><img src={logo} alt="" /><strong>Hire<br />AI</strong></div>
    ...
    <main><header className="topbar"><button className="icon menu" onClick={() => setOpen(true)} aria-label="Open navigation"><Menu /></button><span>Talent intelligence workspace</span><span className="status"><i /> Systems ready</span></header>{children}<RoleAssistant role={user.role} /></main>
  </div>;
}
...
function Applications() {
  ...
  useEffect(() => { api("/candidate/applications").then(setItems).catch(requestError => setError(requestError.message)); }, []);
  const view = useListView(items, "applied_at", "status");
  return <Page eyebrow="Your search" title="Application tracker">{error && <p className="error">{error}</p>}<ListControls view={view} sortOptions={[["applied_at", "Applied date"], ["job_title", "Job title"], ["company_name", "Company"], ["status", "Status"]]} placeholder="Search role, company, or status..." /><div className="list">{view.visible.map(item => <div className="job-row application-tracker" key={item.application_id}><div><span className="pill">{item.status}</span><h3>{item.job_title}</h3><p>{item.company_name} · Resume #{item.resume_id}</p>{item.interview_scheduled_at && <p className="application-interview">Interview {item.interview_status?.toLowerCase()} · {new Date(item.interview_scheduled_at).toLocaleString()}</p>}</div><div className="application-date"><small>APPLIED</small><strong>{new Date(item.applied_at).toLocaleDateString()}</strong></div></div>)}</div>{!view.visible.length && <Empty title="No matching applications" text="Submitted roles appear here with their current status." />}</Page>;
}

function Applicants() {
  const { jobId } = useParams(); const [items, setItems] = useState([]); const [interviewers, setInterviewers] = useState([]); const [selectedApplication, setSelectedApplication] = useState(""); const [interviewerId, setInterviewerId] = useState(""); const [scheduledAt, setScheduledAt] = useState(""); const [scheduleMessage, setScheduleMessage] = useState(""); const [error, setError] = useState(""); const [loading, setLoading] = useState(true); const [verifying, setVerifying] = useState(false); const [progress, setProgress] = useState(null); const [graph, setGraph] = useState(null); const [graphLoading, setGraphLoading] = useState(null);
  async function loadApplicants() { return api(`/recruiter/jobs/${jobId}/applicants`).then(setItems); }
  useEffect(() => { Promise.all([api(`/recruiter/jobs/${jobId}/applicants`), api("/recruiter/interviewers")]).then(([applicants, staff]) => { setItems(applicants); setInterviewers(staff); if (staff.length) setInterviewerId(String(staff[0].interviewer_id)); }).catch(err => setError(err.message)).finally(() => setLoading(false)); }, [jobId]);
  async function verifyAndRerank() { setVerifying(true); setError(""); setProgress({ status: "RUNNING", stage: "PREPARING", processed: 0, total: 0, github_processed: 0, llm_processed: 0, percent: 0 }); const timer = window.setInterval(() => api(`/recruiter/jobs/${jobId}/match-progress`).then(setProgress).catch(() => {}), 750); try { await api(`/recruiter/jobs/${jobId}/match`, { method: "POST" }); await loadApplicants(); setProgress(await api(`/recruiter/jobs/${jobId}/match-progress`)); } catch (err) { setError(err.message); setProgress(current => ({ ...current, status: "FAILED", stage: "FAILED" })); } finally { window.clearInterval(timer); setVerifying(false); } }
  ...
  async function scheduleInterview(event) { event.preventDefault(); setScheduleMessage(""); try { await api("/recruiter/interviews", { method: "POST", body: JSON.stringify({ application_id: Number(selectedApplication), interviewer_id: Number(interviewerId), scheduled_at: new Date(scheduledAt).toISOString() }) }); setScheduleMessage("Interview scheduled successfully."); setSelectedApplication(""); await loadApplicants(); } catch (requestError) { setScheduleMessage(requestError.message); } }
  const view = useListView(items, "ranking", "scope");
  return <Page eyebrow={`Role #${jobId}`} title="Ranked applicants" action={<button className="primary" onClick={verifyAndRerank} disabled={verifying || loading}><GitBranch />{verifying ? "Verifying projects..." : "Verify GitHub & rerank"}</button>}>
    {loading && <p className="notice">Indexing resumes and calculating semantic matches...</p>}
    {progress && progress.status !== "IDLE" && <section className={`verification-progress ${progress.status.toLowerCase()}`} aria-live="polite">
      <div className="verification-progress-head"><div><p className="eyebrow">GitHub verification progress</p><h3>{progress.stage.replaceAll("_", " ").toLowerCase()}</h3></div><strong>{progress.percent}%</strong></div>
      <div className="verification-track" role="progressbar" aria-label="Applicants processed" aria-valuemin="0" aria-valuemax="100" aria-valuenow={progress.percent}><i style={{ width: `${progress.percent}%` }} /></div>
      <div className="verification-counts"><span><small>APPLICANTS</small><strong>{progress.processed} / {progress.total || "..."}</strong></span><span><small>GITHUB CHECKS</small><strong>{progress.github_processed}</strong></span><span><small>AI EXPLANATIONS</small><strong>{progress.llm_processed}</strong></span><span><small>STATUS</small><strong>{progress.status}</strong></span></div>
    </section>}
    {error && <p className="error">{error}</p>}
    <form className="schedule-panel" onSubmit={scheduleInterview}><label>Selected applicant<select value={selectedApplication} onChange={event => setSelectedApplication(event.target.value)} required><option value="">Select from list</option>{items.map(item => <option key={item.application_id} value={item.application_id}>{item.candidate_name}</option>)}</select></label><label>Interviewer<select value={interviewerId} onChange={event => setInterviewerId(event.target.value)} required><option value="">Select interviewer</option>{interviewers.map(item => <option key={item.interviewer_id} value={item.interviewer_id}>{item.name} · {item.designation || "Interviewer"}</option>)}</select></label><label>Date and time<input type="datetime-local" value={scheduledAt} onChange={event => setScheduledAt(event.target.value)} required /></label><button className="primary" disabled={!selectedApplication || !interviewerId || !scheduledAt}>Schedule interview</button>{scheduleMessage && <p className={scheduleMessage.includes("successfully") ? "success" : "error"}>{scheduleMessage}</p>}</form>
    <ListControls view={view} sortOptions={[["ranking", "Rank"], ["overall_score", "Overall score"], ["semantic_score", "Semantic score"], ["github_score", "GitHub relevance"], ["candidate_name", "Candidate name"], ["applied_at", "Applied date"]]} placeholder="Search candidate or resume..." />
    <div className="list">{view.visible.map(item => <div className={`job-row applicant-row ${String(item.application_id) === selectedApplication ? "selected" : ""}`} key={item.application_id}><label className="applicant-select"><input type="radio" name="selected_application" checked={String(item.application_id) === selectedApplication} onChange={() => setSelectedApplication(String(item.application_id))} /><span>Select</span></label><div className="applicant-rank"><strong>{item.ranking ? `#${item.ranking}` : "--"}</strong><small>RANK</small></div><div className="applicant-copy"><span className="pill">{item.status}</span><h3><button className="applicant-name" onClick={() => openGraph(item)} disabled={graphLoading === item.candidate_id}>{graphLoading === item.candidate_id ? "Loading graph..." : item.candidate_name}</button></h3><p>Submitted resume #{item.resume_id} - {new Date(item.applied_at).toLocaleDateString()}</p>{item.github_evidence && <small className={item.github_verified ? "github-verified" : "github-unverified"}>{item.github_verified ? `GitHub verified · ${item.github_score?.toFixed(1) ?? 0}% project relevance` : `GitHub: ${item.github_evidence.verification_status?.replaceAll("_", " ").toLowerCase()}`}</small>}</div><div className={`match-score ${item.semantic_score == null ? "pending" : ""}`}><strong>{item.semantic_score == null ? "Pending" : `${(item.overall_score ?? item.semantic_score).toFixed(1)}%`}</strong><small>{item.overall_score == null ? "SEMANTIC MATCH" : "FINAL MATCH"}</small>{item.overall_score != null && <span>{item.semantic_score.toFixed(1)} semantic</span>}</div></div>)}</div>
    {!view.visible.length && !error && !loading && <Empty title="No matching applicants" text="Adjust the search, scope, or sorting controls." />}{graph && <ApplicantGraphModal data={graph} onClose={() => setGraph(null)} />}
  </Page>;
}

function Interviews() {
  const [items, setItems] = useState([]); const [explanation, setExplanation] = useState(null); const [feedbackItem, setFeedbackItem] = useState(null); const [loadingId, setLoadingId] = useState(null); const [error, setError] = useState("");
  const loadInterviews = () => api("/interviewer/candidates").then(setItems);
  useEffect(() => { loadInterviews().catch(requestError => setError(requestError.message)); }, []);
  ...
  async function submitFeedback(event) { event.preventDefault(); const form = new FormData(event.currentTarget); try { await api(`/interviewer/interviews/${feedbackItem.interview_id}/feedback`, { method: "POST", body: JSON.stringify({ score: Number(form.get("score")), feedback: form.get("feedback") }) }); setFeedbackItem(null); await loadInterviews(); } catch (requestError) { setError(requestError.message); } }
  const view = useListView(items, "scheduled_at", "status");
  return <Page eyebrow="Assigned conversations" title="Interview queue">{error && <p className="error">{error}</p>}<ListControls view={view} sortOptions={[["scheduled_at", "Schedule"], ["candidate_name", "Candidate"], ["job_title", "Job role"], ["score", "Score"]]} placeholder="Search candidate, role, or application..." /><div className="list">{view.visible.map(item => <div className="job-row interview-row" key={item.interview_id}><div><span className="pill">{item.status}</span><h3>{item.candidate_name || `Application #${item.application_id}`}</h3><p>{item.job_title || "Job role"} · {item.scheduled_at ? new Date(item.scheduled_at).toLocaleString() : "Schedule pending"}</p>{item.feedback && <p className="interview-feedback">{item.feedback}</p>}</div><div className="interview-actions"><strong>{item.score == null ? "Not scored" : `${item.score}/100`}</strong><button className="secondary" onClick={() => explain(item)} disabled={loadingId === item.interview_id}><Lightbulb />{loadingId === item.interview_id ? "Analyzing..." : "Why suitable?"}</button>{item.status !== "COMPLETED" && <button className="primary" onClick={() => setFeedbackItem(item)}>Add feedback</button>}</div></div>)}</div>{!view.visible.length && <Empty title="No matching interviews" text="Scheduled candidate conversations appear here." />}{feedbackItem && <div className="modal-backdrop"><form className="admin-modal feedback-modal" onSubmit={submitFeedback}><button type="button" className="icon modal-close" onClick={() => setFeedbackItem(null)}><X /></button><p className="eyebrow">Interview outcome</p><h2>{feedbackItem.candidate_name}</h2><label>Score (0-100)<input name="score" type="number" min="0" max="100" step="0.1" required /></label><label>Job-relevant feedback<textarea name="feedback" rows="6" minLength="3" maxLength="5000" required /></label><button className="primary">Complete interview</button></form></div>}{explanation && <SuitabilityModal data={explanation} onClose={() => setExplanation(null)} />}</Page>;
}
